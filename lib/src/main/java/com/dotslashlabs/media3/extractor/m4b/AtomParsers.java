 ...
 * Copyright (C) 2023 Shirish Kamath (https://github.com/dotslashlabs)
 *
...
/* package */ final class AtomParsers {
  ...
  public static final class UdtaInfo {
    ...
    /** The chapters metadata retrieved from the chpl sub atom. */
    @Nullable public final Metadata chplMetadata;
  ...
  public static UdtaInfo parseUdta(Atom.LeafAtom udtaAtom) {
    ...
    @Nullable Metadata chplMetadata = null;
    ...
    while (udtaData.bytesLeft() >= Atom.HEADER_SIZE) {
      ...
      } else if (atomType == Atom.TYPE_chpl) {
        udtaData.setPosition(atomPosition);
        chplMetadata = parseChapters(udtaData, atomPosition + atomSize);
    ...
    return new UdtaInfo(metaMetadata, smtaMetadata, xyzMetadata, chplMetadata);
  }
  ...
  private static TrackSampleTable parseStbl(
    ...
    if (rechunkFixedSizeSamples) {
      ...
      com.dotslashlabs.media3.extractor.m4b.FixedSampleSizeRechunker.Results rechunkedResults =
          com.dotslashlabs.media3.extractor.m4b.FixedSampleSizeRechunker.rechunk(
  ...
  /**
   * Parses metadata from a chpl atom.
   */
  private static Metadata parseChapters(ParsableByteArray chpl, @SuppressWarnings("UNUSED_PARAMETER") int limit) {
    chpl.skipBytes(Atom.HEADER_SIZE);

    int version = chpl.readUnsignedByte(); // 8
    long flags = chpl.readUnsignedInt(); // 32
    long chapterEntries = chpl.readUnsignedInt(); // 32

    List<ChapterMetadata> chapters = new ArrayList<>();

    if (chapterEntries > 0 && chapterEntries < Integer.MAX_VALUE) {
      for (int i = 0; i < chapterEntries; i++) {
        long startTime = chpl.readLong(); // 64
        int titleByteLength = chpl.readUnsignedByte(); // 8
        String chapterTitle = null;

        if (titleByteLength > 0) {
          chapterTitle = chpl.readString(titleByteLength);
        }

        chapters.add(new ChapterMetadata(startTime, chapterTitle));
      }

      chapters.sort(Comparator.comparingLong(ChapterMetadata::getStartTime));
    } else {
      Log.w(TAG, "Skipping chapters. Incorrect count: " + chapterEntries);

      for (int i = 0; i < chapterEntries; i++) {
        chpl.skipBytes(8); // startTime
        int titleByteLength = chpl.readUnsignedByte();
        chpl.skipBytes(titleByteLength);
      }
    }

    return new Metadata(new ChapterListFrame(version, flags, chapterEntries, chapters));
  }
  ...
  private static void parseVideoSampleEntry(
    ...
    if (atomType == Atom.TYPE_encv) {
      @Nullable
      Pair<Integer, com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox> sampleEntryEncryptionData =
   ...
   * unencrypted atom type and a {@link com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox}. Null is returned if no common
   * encryption sinf atom was present.
   */
  @Nullable
  private static Pair<Integer, com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox> parseSampleEntryEncryptionData(
    ...
    while (childPosition - position < size) {
      ...
      if (childAtomType == Atom.TYPE_sinf) {
        @Nullable
        Pair<Integer, com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox> result =
  ...
  @Nullable
  /* package */ static Pair<Integer, com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox> parseCommonEncryptionSinfFromParent(
      ParsableByteArray parent, int position, int size) throws ParserException {
    ...
    if (C.CENC_TYPE_cenc.equals(schemeType)
      ...
      @Nullable
      com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox encryptionBox =
          parseSchiFromParent(
  ...
  @Nullable
  private static com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox parseSchiFromParent(
    ...
    while (childPosition - position < size) {
      ...
      if (childAtomType == Atom.TYPE_tenc) {
        ...
        return new com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox(
            defaultIsProtected,
            schemeType,
  ...
  private static final class StsdData {
    ...
    public final com.dotslashlabs.media3.extractor.m4b.TrackEncryptionBox[] trackEncryptionBoxes;
